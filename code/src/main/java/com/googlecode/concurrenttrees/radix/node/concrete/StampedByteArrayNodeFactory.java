 ...
 */
public class StampedByteArrayNodeFactory implements StampedNodeFactory {
    ...
    public AtomicStampedReference<Node> createStampedNode(CharSequence edgeCharacters, Object value, List<Node> childNodes, boolean isRoot) {
        ...
        if (childNodes.isEmpty()) {
            // Leaf node...
            if (value instanceof VoidValue) {
                return new AtomicStampedReference<Node>( new ByteArrayNodeLeafVoidValue(edgeCharacters),0);
            }
            else if (value != null) {
                return new AtomicStampedReference<Node>( new ByteArrayNodeLeafWithValue(edgeCharacters, value),0);
            }
            else {
                return new AtomicStampedReference<Node>( new ByteArrayNodeLeafNullValue(edgeCharacters),0);
            }
        }
        else {
            // Non-leaf node...
            if (value instanceof VoidValue) {
                return new AtomicStampedReference<Node>( new ByteArrayNodeNonLeafVoidValue(edgeCharacters, childNodes),0);
            }
            else if (value == null) {
                return new AtomicStampedReference<Node>( new ByteArrayNodeNonLeafNullValue(edgeCharacters, childNodes),0);
            }
            else {
                return new AtomicStampedReference<Node>( new ByteArrayNodeDefault(edgeCharacters, value, childNodes),0);
            }
        }
    }
	...
	public Node createNode(CharSequence edgeCharacters, Object value,
			AtomicStampedReference<Node>[] childNodes, boolean isRoot) {
