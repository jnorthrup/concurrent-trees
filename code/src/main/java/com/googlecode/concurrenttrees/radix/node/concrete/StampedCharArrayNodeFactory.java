 ...
 */
public class StampedCharArrayNodeFactory implements StampedNodeFactory {
    ...
    public AtomicStampedReference<Node> createStampedNode(CharSequence edgeCharacters, Object value, List<Node> childNodes, boolean isRoot) {
        ...
        if (childNodes.isEmpty()) {
            ...
            if (value instanceof VoidValue) {
                return new AtomicStampedReference<Node>( new CharArrayNodeLeafVoidValue(edgeCharacters), 0);
            }
            else if (value != null) {
                return new AtomicStampedReference<Node>(new CharArrayNodeLeafWithValue(edgeCharacters, value), 0);
            }
            else {
                return new AtomicStampedReference<Node>(new CharArrayNodeLeafNullValue(edgeCharacters), 0);
            }
        }
        else {
            // Non-leaf node...
            if (value instanceof VoidValue) {
                return new AtomicStampedReference<Node>(new CharArrayNodeNonLeafVoidValue(edgeCharacters, childNodes), 0);
            }
            else if (value == null) {
                return new AtomicStampedReference<Node>( new CharArrayNodeNonLeafNullValue(edgeCharacters, childNodes), 0);
            }
            else {
                return new AtomicStampedReference<Node>(new CharArrayNodeDefault(edgeCharacters, value, childNodes),0);
            }
        }
    }
	...
	@Override
	public Node createNode(CharSequence edgeCharacters, Object value,
			AtomicStampedReference<Node>[] finalEdges, boolean isRoot) {
		if (finalEdges == null) {
            throw new IllegalStateException("The edgeCharacters argument was null");
        }
        if (!isRoot && finalEdges.length == 0) {
            throw new IllegalStateException("Invalid edge characters for non-root node: " + CharSequences.toString(edgeCharacters));
        }
        if (finalEdges == null) {
            throw new IllegalStateException("The childNodes argument was null");
        }
        NodeUtil.ensureNoDuplicateEdges(finalEdges);
        if (finalEdges.length==0) {
            ...
            // Non-leaf node...
            if (value instanceof VoidValue) {
                return new CharArrayNodeNonLeafVoidValue(edgeCharacters, finalEdges);
            }
            else if (value == null) {
                return new CharArrayNodeNonLeafNullValue(edgeCharacters, finalEdges);
            }
            else {
                return new CharArrayNodeDefault(edgeCharacters, value, finalEdges);
            }
        }
	}
